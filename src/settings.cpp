...
// The default neutron energy threshold is based on the recommendations
// of J. P. Morgan et al. See Section 3.3 in:
    ...
    if (delta_tracking) {
      ...
      if (check_for_node(delta_tracking_node, "hybrid_type")) {
        ...
        if (hybrid_type == "cross_section") {
          ...
          write_message("Running delta tracking with the hybrid-in-cross-section scheme.");
        } else if (hybrid_type == "energy") {
          hybrid_delta_type = HybridTrackingType::Energy;
          write_message("Running delta tracking with the hybrid-in-energy scheme.");
        } else {
      ...
      if (hybrid_delta_type == HybridTrackingType::CrossSection) {
        write_message("Using a local cross section threshold of " + std::to_string(hybrid_xs_threshold), 0);
      }
      ...
      if (hybrid_delta_type == HybridTrackingType::Energy) {
        ...
        write_message("Using a neutron energy threshold of " + std::to_string(hybrid_energy_threshold[i_neutron]), 0);
        write_message("Using a photon energy threshold of " + std::to_string(hybrid_energy_threshold[i_photon]), 0);
      }
